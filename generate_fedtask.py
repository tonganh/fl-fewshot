...
def read_option():
    ...
    parser.add_argument(
        "--number_class_per_client",
        help="the number of classes per client;",
        type=int,
        default=2,
    )
    try:
...
if __name__ == "__main__":
    option = read_option()
    TaskGen = getattr(
        importlib.import_module(".".join(["benchmark", option["dataset"], "core"])),
        "TaskGen",
    )
    generator = TaskGen(
        dist_id=option["dist"],
        ...
        number_class_per_client=option["number_class_per_client"],
    )
    # generator = TaskGen(
    #     dist_id=option["dist"],
    #     skewness=option["skew"],
