...
class MyLogger(flw.Logger):
    def log(self, server=None):
        ...
        breakpoint()
        valid_metrics, valid_losses = server.test_on_clients(
            self.current_round, "valid"
        )
        train_metrics, train_losses = server.test_on_clients(
            self.current_round, "train"
        )
        self.output["train_losses"].append(
            1.0
            * sum([ck * closs for ck, closs in zip(server.client_vols, train_losses)])
            / server.data_vol
        )
        ...
        self.output["mean_valid_accs"].append(
            1.0
            * sum([ck * acc for ck, acc in zip(server.client_vols, valid_metrics)])
            / server.data_vol
        )
        ...
        for cid in range(server.num_clients):
            self.output["client_accs"][server.clients[cid].name] = [
                self.output["valid_accs"][i][cid]
                for i in range(len(self.output["valid_accs"]))
            ]
